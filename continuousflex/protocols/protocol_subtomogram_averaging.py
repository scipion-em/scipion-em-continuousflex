...
class FlexProtSubtomogramAveraging(ProtAnalysis3D):
    ...
    The second mode is to import a previously performed alignment using this protocol,
    Dynamo, Artiatomi, TOM Toolbox, EMAN2, or compatible metadata sources in order
    to reconstruct a consensus average from existing alignment parameters.

    AI Generated:

    Subtomogram Averaging (FlexProtSubtomogramAveraging) - User Manual
        Overview

        The Subtomogram Averaging protocol is designed to generate high-quality
        three-dimensional averages from collections of subtomograms obtained in
        cryo-electron tomography experiments. Its primary objective is to improve
        the signal-to-noise ratio by bringing structurally related particles into
        a common orientation and combining them into a representative consensus
        structure. This process is a central step in many tomography workflows
        because individual subtomograms are often too noisy to allow detailed
        structural interpretation on their own.

        The protocol supports both complete averaging workflows and workflows that
        begin from previously aligned data. This flexibility allows users to
        perform de novo subtomogram averaging within the same environment or to
        continue processing results generated by external software packages.

        Inputs and General Workflow

        The protocol requires a collection of subtomograms representing multiple
        observations of the same biological object or molecular assembly. All
        subtomograms should ideally have consistent dimensions and sampling rates
        to ensure meaningful averaging and downstream interpretation.

        Depending on the selected workflow, the protocol either determines the
        alignment parameters automatically through iterative refinement or imports
        previously calculated orientations and shifts. In both cases, the final
        objective is the generation of a consensus average that represents the
        common structural features present across the dataset.

        Performing Subtomogram Averaging with Fast Rotational Matching

        In its native averaging mode, the protocol uses Fast Rotational Matching
        to iteratively align subtomograms against a reference volume. The process
        starts from either an automatically generated reference, an externally
        supplied volume, or a volume selected from the current workspace.

        Iterative refinement progressively improves the reference by repeatedly
        aligning all particles and recalculating the average. This approach is
        particularly useful when the dataset contains substantial noise but the
        underlying biological structure is sufficiently consistent across the
        particles. As iterations proceed, common structural features become more
        pronounced while random noise is reduced.

        The number of refinement cycles determines how extensively the dataset is
        optimized. Moderate values are often sufficient for routine analyses,
        whereas challenging datasets may benefit from additional iterations.

        Starting References and Their Biological Importance

        The choice of starting reference can strongly influence convergence and
        the quality of the final average. Beginning from scratch is often
        appropriate when no reliable template exists and allows the protocol to
        generate an unbiased initial estimate from the available particles.

        Providing a biologically relevant starting reference may accelerate
        convergence and improve robustness, especially when dealing with noisy
        datasets. However, users should remain aware of the potential for
        reference bias and should always verify that the resulting average is
        supported by the experimental data.

        Missing-Wedge Compensation

        A major challenge in cryo-electron tomography is the missing-wedge effect,
        which arises from incomplete angular sampling during data acquisition.
        This anisotropic loss of information can influence alignment quality and
        distort structural interpretation.

        The protocol provides an option to compensate for the missing wedge during
        alignment. This compensation is generally beneficial when working with
        raw or newly aligned subtomograms because it reduces orientation bias and
        improves the consistency of the resulting average.

        In contrast, when alignment parameters have already been generated by
        external software or previous processing stages, additional compensation
        may not be necessary. Users should select the strategy that best matches
        the origin and processing history of their data.

        Masking and Focused Averaging

        The protocol allows the use of a three-dimensional mask during alignment.
        From a biological perspective, masking can be one of the most important
        factors influencing averaging quality because it determines which regions
        contribute most strongly to the alignment process.

        Well-designed masks typically encompass the structurally stable portion of
        the particle while excluding surrounding solvent, reconstruction artifacts,
        or highly flexible regions. Focusing the alignment on conserved features
        often improves convergence and produces averages that better represent the
        biologically relevant core of the structure.

        Care should be taken to avoid masks that remove genuine structural
        information or artificially constrain the alignment.

        Importing Existing Alignments

        Many tomography projects already contain alignment results generated by
        specialized packages. To facilitate interoperability, the protocol can
        import previously determined rigid-body transformations from multiple
        external sources.

        This capability allows researchers to validate imported alignments,
        reconstruct consensus averages, and continue downstream analyses within a
        unified workflow. Reproducing a known average from imported parameters is
        often a useful verification step before proceeding with refinement,
        classification, or heterogeneity studies.

        The imported alignment information is converted into a common framework so
        that subsequent processing remains consistent regardless of the original
        software used to generate the orientations and shifts.

        Averaging Pre-Aligned Subtomograms

        For datasets that have already been aligned, the protocol can generate a
        consensus average directly without performing additional refinement. This
        mode is useful when alignment quality has already been validated and the
        user simply wishes to obtain an averaged reconstruction.

        Such workflows are common when subtomograms originate from external
        pipelines, collaborative projects, or previously completed processing
        stages.

        Outputs and Their Interpretation

        The primary output is a subtomogram average representing the common
        structural signal present across the input particles. This volume can be
        used for visualization, structural interpretation, flexible analysis,
        classification, refinement, or integration with other cryo-EM workflows.

        When alignment has been performed or imported, the associated orientation
        and translation parameters are preserved as part of the workflow, enabling
        reproducibility and further downstream processing.

        The biological quality of the resulting average depends not only on the
        number of particles but also on their structural homogeneity. Averaging
        particles belonging to different conformational states may blur important
        features and obscure meaningful biological variability.

        Practical Recommendations

        For most studies, it is advisable to begin with careful inspection of the
        input subtomograms and to ensure that all particles correspond to the same
        molecular complex or structural state. Appropriate masking and realistic
        missing-wedge settings often provide substantial improvements in alignment
        stability and final map quality.

        When a reliable template is available, it can accelerate convergence and
        improve robustness. When no trustworthy template exists, starting from an
        unbiased average is often preferable.

        Imported alignments should always be validated by examining whether the
        reconstructed average is consistent with previously obtained results.

        Final Perspective

        Subtomogram averaging transforms collections of noisy tomographic particles
        into biologically interpretable three-dimensional structures. Whether used
        for iterative alignment, reconstruction from imported parameters, or direct
        averaging of pre-aligned data, the protocol provides a flexible framework
        for extracting structural information from cryo-electron tomography
        datasets. Careful consideration of reference selection, masking strategy,
        missing-wedge treatment, and particle homogeneity is essential for
        obtaining reliable and biologically meaningful averages.
    """
